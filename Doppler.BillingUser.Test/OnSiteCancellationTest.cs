...
{
    public class OnSiteCancellationTest : IClassFixture<WebApplicationFactory<Startup>>
    {
        ...
        public OnSiteCancellationTest(WebApplicationFactory<Startup> factory)
        {
            _factory = factory;
        }
        ...
        [InlineData("")]
        [InlineData("invalid")]
        public async Task PUT_cancel_onsite_plan_should_return_unauthorized_When_token_is_invalid(string token)
        {
            // Arrange
            ...
            var request = new HttpRequestMessage(HttpMethod.Put, $"accounts/{accountname}/onsite/cancel")
            {
        ...
        [Fact]
        public async Task PUT_cancel_onsite_plan_should_return_not_found_when_user_not_exists()
        {
            // Arrange
            var accountname = "test1@example.com";
        ...
        [Fact]
        public async Task PUT_cancel_onsite_plan_should_return_not_found_when_user_not_have_any_plan()
        {
            // Arrange
            var accountname = "test1@example.com";
            ...
            // Assert
            Assert.Equal(HttpStatusCode.NotFound, response.StatusCode);
            Assert.Equal("The user does not have any onsite plan", messageError);

            userAddOnRepository.Verify(x => x.GetByUserIdAndAddOnType(
                expectedUser.IdUser,
                (int)AddOnType.OnSite), Times.Once());
        }

        [Fact]
        public async Task PUT_cancel_onsite_plan_should_return_ok_when_information_is_correct()
        {
            // Arrange
            var accountname = "test1@example.com";
            var expectedUser = new User() { IdUser = 1 };
            var expectedUserAddOn = new UserAddOn() { IdCurrentBillingCredit = 123 };
            var expectedBillingCredit = new BillingCredit() { IdBillingCredit = 123, IdBillingCreditType = (int)BillingCreditTypeEnum.OnSite_Buyed_CC };
            ...
            Assert.Equal($"REG - Successful cancel onsite plan for: User: {accountname}", messageError);
            ...
            billingRepository.Verify(x => x.UpdateBillingCreditType(
                ...
                (int)BillingCreditTypeEnum.OnSite_Canceled), Times.Once());
        }
    }
}
